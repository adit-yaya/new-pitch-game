...
class Game(object):
    ...
    friendlyAliens = ["A", "D#", "C#", "G", "B", "F"]
    enemyAliens = ["A#", "E", "D", "G#", "C", "F#"]
    numCapturedAliens = 0
    ...
    def __init__(self, VERSION):
        ...
        # self.captureList = []
        # self.bothList = []
        # while(Level.currentLevel<6):
        #     self.shootList.append(choice(level_dict[Level.currentLevel]))
        # while(Level.currentLevel):
        #     self.captureList = level_dict[Level.currentLevel]
        #     self.targets = self.captureList
    ...
    def display_frame(self, screen):
        ...
        if self.newBlock:  
            ...
            text = font.render("Game Over, you successfully completed "+ str(Level.currentLevel) +
                               " levels", True, GREEN)
            center_text(text)
            text2 = font.render("You scored %d out of %d possible points"%(self.score, self.numberAliens * 20),
                                True, GREEN)
            center_x = (SCREEN_WIDTH // 2) - (text2.get_width() // 2)
            center_y = (SCREEN_HEIGHT // 2) + (text2.get_height() // 2) + 2
            screen.blit(text2, [center_x, center_y])
