...
class Enemy(pygame.sprite.Sprite):
    ...
    enemyA_images = sorted(["Images/Enemies/EnemyA/{0}".format(i) for i in enemyA_images if not i.startswith('.')])
    enemyB_images = listdir("Images/Enemies/EnemyB")
    enemyB_images = sorted(["Images/Enemies/EnemyB/{0}".format(i) for i in enemyB_images if not i.startswith('.')])
    offscreen_time = 3 #seconds before appearance
    enemySightTime = []
