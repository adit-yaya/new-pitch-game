...
def main():
    def checkData(subj):
        if VERSION==1:
            if path.exists("Subject %s/Variance/"%subj):
                subj = input("Data already exists for that subject, Please choose a different subject number: ")
    ...
    VERSION = int(input("Condition (1 or 2): "))
    SUBJECT = checkData(subj = input("Subject Number: "))
    ...
    print(NewGame.gameData)
    #general = pd.DataFrame(NewGame.gameData)
