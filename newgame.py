...
class NewGame(object):
    ...
    def __init__(self, VERSION):
        ...
        # for the very start of the game, and establishing menus
        self.gameMenu = True
        self.captureMenu = False
        self.bothMenu = False
        self.gamePlaying = False             
        self.currentLevel = 1
        
        # establish where enemies are going to be pulled from
        self.alienList = level_dict[self.currentLevel] + level_dict[self.currentLevel]
        random.shuffle(self.alienList)
    ...
    def process_events(self):
        ...
        def shoot(color, target, degree, origin):
            ...
            if self.ammo > 0:
                ...
                self.shot_times.append(pygame.time.get_ticks)
            else:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if self.newPhase:
                        ...
                        if self.phaseCount == 2:
                            self.captureMenu = True
                        elif self.phaseCount == 3:
                            self.bothMenu = True
    ...
    def run_logic(self):
        ...
        def check_level():
            ...
            if len(self.alienList) == 0:
                if self.numElimAliens >= 0.8 * self.aliensPerLevel:
                    self.currentLevel += 1
                    check_phase()
                    self.alienList = level_dict[self.currentLevel] + level_dict[self.currentLevel]
                    random.shuffle(self.alienList) 
                    self.aliensPerLevel = len(self.alienList)
                    self.numElimAliens = 0
                    Enemy.speed += (1/FPS) * 60
                else:            #if failed level, exc. for level 1
                    self.alienList = level_dict[self.currentLevel] + level_dict[self.currentLevel]
                    random.shuffle(self.alienList) 
                    self.aliensPerLevel = len(self.alienList)
                    self.numElimAliens = 0
        ...
        if self.gamePlaying:
            ...
            if not self.enemy_live and self.elapsedTime==self.enemySpawnTime:
                ...
                self.enemy_type = self.alienList.pop()
